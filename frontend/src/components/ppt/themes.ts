...
export type ThemeTokens = {
  ...
  mesh?: string
  archetype?: 'ink' | 'editorial' | 'illustrated'
}
...
export const THEMES: Record<PptTemplateId, ThemeTokens> = {
  academic: {
    ...
    border: 'rgba(32,37,33,0.12)',
    titleSize: 'clamp(1.75rem, 3.2vw, 2.75rem)',
    bodySize: 'clamp(1rem, 1.5vw, 1.35rem)',
    radius: '0.25rem',
    shadow: '0 20px 60px rgba(32,37,33,0.08)',
    fontDisplay: '"Noto Serif SC", "Songti SC", serif',
    chip: '#0F766E',
    mesh: 'linear-gradient(90deg,rgba(49,92,78,.08) 1px,transparent 1px),linear-gradient(rgba(49,92,78,.04) 1px,transparent 1px)',
    archetype: 'editorial',
  },
  classroom: {
    id: 'classroom',
    bg: 'linear-gradient(160deg, #FFF9E9 0%, #F4F0DC 100%)',
    fg: '#34312D',
    muted: '#776F66',
    accent: '#D7654B',
    accentSoft: '#FBE4D5',
    panel: 'rgba(255,255,255,0.62)',
    border: 'rgba(120,92,66,0.16)',
    titleSize: 'clamp(1.7rem, 3vw, 2.5rem)',
    bodySize: 'clamp(1rem, 1.4vw, 1.3rem)',
    radius: '1rem',
    shadow: '0 18px 50px rgba(116,84,55,0.12)',
    fontDisplay: '"Noto Serif SC", "Songti SC", serif',
    chip: '#D7654B',
    mesh: 'radial-gradient(circle at 8% 12%,rgba(224,122,95,.16) 0 42px,transparent 43px),radial-gradient(circle at 92% 84%,rgba(129,178,154,.18) 0 70px,transparent 71px)',
  ...
  sage: {
    id: 'sage',
    /** 宣纸米白 + 墨色 + 朱砂点缀 + 竹简肌理 */
    bg: 'linear-gradient(165deg, #F7F0E2 0%, #EEE4D2 55%, #E5D8C1 100%)',
    fg: '#24211D',
    muted: '#746B5E',
    accent: '#8E2F2B',
    accentSoft: 'rgba(142,47,43,0.09)',
    panel: 'rgba(255,252,245,0.34)',
    ...
    chip: '#9F1239',
    mesh: 'radial-gradient(ellipse at 86% 18%,rgba(34,45,37,.13),transparent 34%),radial-gradient(ellipse at 10% 88%,rgba(34,45,37,.08),transparent 27%),repeating-linear-gradient(0deg,transparent,transparent 3px,rgba(71,55,37,.018) 3px,rgba(71,55,37,.018) 4px)',
    archetype: 'ink',
  },
  coral: {
    id: 'coral',
